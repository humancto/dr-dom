...
class GDPRCCPAChecker {
  ...
  checkOptOut() {
    ...
    for (const pattern of optOutPatterns) {
      // Find links and buttons containing the pattern text
      const links = Array.from(document.querySelectorAll('a, button')).find(el => 
        el.textContent.toLowerCase().includes(pattern)
      );
      
      if (links || document.body.textContent.toLowerCase().includes(pattern)) {
        optOutFound = true;
        break;
      }
    }
  ...
  findButton(patterns) {
    for (const pattern of patterns) {
      // Find buttons and links containing the pattern text
